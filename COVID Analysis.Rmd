...
This is an on-going analysis aimed at tracking the impact of COVID-19 on the City of New York (NYC). Using publicly available data provided by the City, this work provides insights into the daily and total number of COVID infections, hospitalizations and deaths in the City. Additionally, it looks at COVID-19 cases among various demographics to determine which ones have been impacted the most.
...
As of `r max(daily$date_of_interest)`, approximately 3.2 million COVID infections have been recorded in NYC, with close to 202,000 of those infection leading to hospitalization. Almost 44,300 people have their lives from the COVID-19 in the City.
...
#
The charts above show that NYC experienced the peak of infection in the beginning of 2022, during the Omicron wave. While there have been three waves in hospitalizations and deaths, most of the hospitalizations and deaths occurred during the initial wave of infections (between March and April of 2020). The availability of vaccines during the Omicron wave helped the number of hospitalizations and deaths around that time.
...
This section disaggregates the daily and total number of COVID cases by borough among the five NYC boroughs.
...
Because these are raw numbers, densely populated boroughs are likely to have higher number of cases. 
...
group %>% 
  ...
  plot_ly(x = ~ Borough, y = ~`Case Count`, type = 'bar', color = ~Indicator, colors = "Set2") %>% 
...
It appears that daily infections, hospitalizations and deaths have consistently been highest in Brooklyn and Queen.
...
daily %>% 
  plot_ly(x = ~date_of_interest, y = ~BX_CASE_COUNT_7DAY_AVG, 
          type = 'scatter', mode = 'line', name = 'Bronx', colors = "Set2") %>% 
...
The table indicates that, even though Brooklyn has had the highest number of COVID cases (see section 6.1), the Bronx has seen the largest share of their cases lead to hospitalizations and deaths. This may be because, although less populated than Brooklyn,  the Bronx has a lot more people with underlying medical conditions that exacerbate the effects of COVID. For example, The Bronx is known to have one of the highest asthma hospitalization rate in the New York State.^[<https://webbi1.health.ny.gov/SASStoredProcess/guest?_program=/EBI/PHIG/apps/asthma_dashboard/ad_dashboard&p=it&ind_id=ad16>]
...
The chart above indicates that after adjusting for population, Staten Island has the highest rate of infections while the Bronx has the highest rate of hospitalizations and deaths.


## 7. Analysing by Age Group

This section details how COVID has affected the various age groups in the City. The data set breaks down age into eight categories - `0-17`, `18-24`, `24-34`, `35-44`, `45-54`, `55-64`, `65-74`, and `75+`.
